...
def main(dataset_arg="mnist_fashion", knn_size_arg="small", iteration=50,
    ...
    tools.save_init_data(save_file=save_file, dataset=dataset, iteration=iteration,
    ...
    if fully:
        if knn_size == "small":
            model = KerasClassifier(build_fn=tools.create_model_small, verbose=0,
                                    use_multiprocessing=multiprocessing_flag, workers=multiprocessing)
        elif knn_size == "medium":
            model = KerasClassifier(build_fn=tools.create_model_medium, verbose=0,
                                    use_multiprocessing=multiprocessing_flag, workers=multiprocessing)
        elif knn_size == "medium":
            model = KerasClassifier(build_fn=tools.create_model_big, verbose=0,
                                    use_multiprocessing=multiprocessing_flag, workers=multiprocessing)
    elif cnn:
        model = KerasClassifier(build_fn=tools.create_model_cnn, verbose=0,
                                use_multiprocessing=multiprocessing_flag, workers=multiprocessing)
    ...
    tools.save_params(dir_path, save_file, means, params, dataset, iteration,
