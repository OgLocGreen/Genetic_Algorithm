...
from tools.evaluation import write_cell
...
def save_params(dir_path, save_file,all_time, means, params, dataset,
                iteration, knn_size, small_dataset, algorithmus,
                acc, precision_score_var, recall_score_var, f1_score_var, cm):
        try:
        ...
        for param in pop:
            if i == 0:
                excel_path = os.path.join(dir_path, "../data/evaluation.xlsx")
                write_cell(path_to_file=os.path.abspath(os.path.realpath(excel_path)), small_dataset=small_dataset, 
