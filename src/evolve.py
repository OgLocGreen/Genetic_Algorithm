...
class Individual(object):
    ...
    def __init__(self, learningrate_min = 0.0005, learningrate_max=0.1, dropout_min=0.05, dropout_max=0.5,
                 epoch_min=5, epoch_max=10, batchsize_min=32, batchsize_max=64):
        self.gene = (random.uniform(learningrate_min, learningrate_max), random.uniform(dropout_min, dropout_max), random.uniform(epoch_min, epoch_max), random.uniform(batchsize_min, batchsize_max))
        self.var_acc = 0
        self.var_loss = 0
    def fitness(self):
        """
...
class Population(object):
    ...
    def select_parents(self):
        ...
        self.individuals = list(sorted(self.individuals, key=lambda x: x.fitness(), reverse=False))
